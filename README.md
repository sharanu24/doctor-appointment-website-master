...
- `MONGODB_URI=mongodb+srv://<username>:<password>@clusterName.xxxxxxx.mongodb.net/doctor-appointment`
 - `MONGODB_URI=mongodb+srv://<username>:<password>@clusterName.xxxxxxx.mongodb.net/<database_name>?retryWrites=true&w=majority`

Note: The backend expects `MONGODB_URI` to be a complete connection string that already contains the database name — the server will use `process.env.MONGODB_URI` directly. Do not append another path manually in code (this caused "Invalid namespace" errors in previous versions).

If your MONGODB_URI does not include a database name, set `MONGODB_DB` instead. The backend will append it safely and fail early with a helpful message if neither is provided.

Developer convenience: for local development (NODE_ENV !== 'production') the backend will fallback to these sensible defaults if you forget to set envs:

- ADMIN_EMAIL=admin@example.com
- ADMIN_PASSWORD=adminPassword123
- JWT_SECRET=devsecret

These fallbacks are intended for local testing only — set proper secure values in production.

Cloudinary (image uploads)
---------------------------
The backend supports uploading images to Cloudinary for doctor profile images and user profile photos. To enable uploads set the following environment variables in `backend/.env`:

```
CLOUDINARY_NAME=your_cloud_name_here
CLOUDINARY_API_KEY=your_cloudinary_api_key_here
CLOUDINARY_SECRET_KEY=your_cloudinary_secret_here
```

If these are missing the server will warn at startup and upload endpoints will return a descriptive error: "Cloudinary not configured..." rather than failing with "Must supply api_key".

For the frontend and admin apps you should also add the backend base URL used by those Vite apps (create `.env` in both `frontend/` and `admin/` directories):
